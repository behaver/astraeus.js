...
class EclipticCoordinate extends CommonCoordinate {
  ...
  from({
    ...
    this.EarthCalculator = new EarthCalculator(this.private.epoch);
  ...
  get(options) {
    if (options === undefined) {
      ...
      this.EarthCalculator.obTime = epoch_0;
  ...
  onGeocentric() {
    if (this.private.centerMode === 'heliocentric') {
      let earth_hecc_sc = this.EarthCalculator.sc;
...
export default EclipticCoordinate;